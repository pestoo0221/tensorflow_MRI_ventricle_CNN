...
IMAGES = [img for img in os.listdir(PATH + '/image') if img.endswith('jpg')]
...
def convoer(inputs, shape,  layer_name):#flag,
    # Create parameters
...
def upconvoer(inputs, inputs_pre, shape, height, width, layer_name):
    # Create parameters
    ...
    with tf.name_scope(layer_name):
       ...
       # Step convolution with inputs
        ...
        concat_img = tf.concat(concat_dim=3, values=[inputs_pre, conv])
       ...
       # Step convolution with inputs
        with tf.name_scope('upconv2'):
            preactivate = conv2d(concat_img, W) + b  #############  INPUTS    ###########
            tf.summary.histogram('pre_activations2', preactivate)
            conv = tf.nn.relu(preactivate, name='activations2')
            tf.summary.histogram('activations2', conv)  
        ...
        with tf.name_scope('upconv3'):
            preactivate = conv2d(conv, Wa) + ba
            tf.summary.histogram('pre_activations3', preactivate)
            conv = tf.nn.relu(preactivate, name='activations3')
            tf.summary.histogram('activations3', conv)
    return conv
...
with tf.Graph().as_default():
    ...
    with tf.name_scope('Model'):
        ...
        conv1 = convoer(inputs, [3,3,1,12], 'layer1')
        
        with tf.name_scope('maxpool1'):
            pool1 = max_pool_2x2(conv1)
            tf.summary.histogram('pool', pool1)

        conv2 = convoer(pool1, [3,3,12,24], 'layer2')
        with tf.name_scope('maxpool2'):
            pool2 = max_pool_2x2(conv2)
            tf.summary.histogram('pool', pool2)

        conv3 = convoer(pool2, [3,3,24,48], 'layer3')
        with tf.name_scope('maxpool3'):
            pool3 = max_pool_2x2(conv3)
            tf.summary.histogram('pool', pool3)

        conv4 = convoer(pool3, [3,3,48,96], 'layer4')
        with tf.name_scope('maxpool4'):
            pool4 = max_pool_2x2(conv4)
            tf.summary.histogram('pool', pool4)

        conv5 = convoer(pool4, [3,3,96,192], 'layer5')

        conv6 = upconvoer(conv5, conv4, [3,3,192,96], 12, 12,'layer6')


        conv7 = upconvoer(conv6, conv3, [3,3,96,48], 24, 24,'layer7')
        conv8 = upconvoer(conv7, conv2, [3,3,48,24], 48, 48,'layer8')
        
        # deconvolution leads to same size image
        conv9 = upconvoer(conv8, conv1, [3,3,24,12], Nsize, Nsize,'layer9')
